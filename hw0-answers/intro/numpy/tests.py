...
def t7(X):
    ...
    """
    return X / np.sum(X*X,1,keepdims=True)**.5
...
def t8(X):
    ...
    """
    return (X - np.mean(X,1,keepdims=True)) / (np.var(X,1,keepdims=True)**.5)
...
def t9(q, k, v):
    ...
    """
    return np.squeeze(v.T @ np.exp(-np.sum((k - q)**2,1,keepdims=True)))
...
def t10(Xs):
    ...
    5) Our 3-line solution uses no loops, and uses the algebraic trick from the
       next problem.
    """
    C = np.array([np.mean(Xs[i],0) for i in range(len(Xs))])
    return t11(C)
...
def t11(X):
    ...
    return np.maximum(xx + xx.T - 2*X@X.T,0) ** .5
...
def t12(X, Y):
    ...
    """
    return (np.sum(X*X,1,keepdims=True) + np.sum(Y*Y,1,keepdims=True).T - 2*X@Y.T ) ** .5
...
def t16(X):
    ...
    """
    return X[:,0:-1] / np.expand_dims(X[:,-1],1)
...
def t18(N, r, x, y):
    ...
    return ((xx-x)**2 + (yy-y)**2 < r**2).astype(float)
...
def t19(N, s, x, y):
    ...
    """
    [xx,yy] = np.meshgrid(np.arange(N),np.arange(N))
    return np.exp(-((xx-x)**2 + (yy-y)**2)/ s**2)
...
def t20(N, v):
    ...
    """
    [xx,yy] = np.meshgrid(np.arange(N),np.arange(N))
    return np.abs((v[0]*xx + v[1]*yy + v[2]) / ((v[0]**2 + v[1]**2)**.5))
